...
import { ASSET_URI_PREFIX, pickAndImportImage } from '../services/assetService';
import { savedSettings } from '../services/settingsService';
import { applyMarkdown, focusStart, insertImage, insertLink, insertTask, localizedEditorExtensions, noteEditorExtensions, type MarkdownFormat } from './editor/noteEditor';
...
defineExpose({
  ...
  insertLink: () => { if (view) insertLink(view); },
  insertImage: async () => {
    if (!view || importingImage) return;
    importingImage = true;
    try {
...
.markdown-editor :deep(.cm-content){min-height:100%;padding:var(--editor-padding-top,6px) 0 20px;caret-color:currentColor;font-synthesis:weight style;user-select:text;-webkit-user-select:text}
...
.markdown-editor :deep(.editor-task-marker){display:inline-flex;box-sizing:border-box;width:28px;height:19px;padding-right:9px;align-items:center;vertical-align:middle;line-height:19px;user-select:none;-webkit-user-select:none}
.markdown-editor :deep(.editor-task-box){display:inline-block;box-sizing:border-box;width:19px;height:19px;margin:0;padding:0;vertical-align:middle;border:1.6px solid currentColor;border-radius:6px;color:inherit;background:transparent;cursor:pointer;font-family:system-ui,sans-serif;line-height:16px;user-select:none;-webkit-user-select:none}
